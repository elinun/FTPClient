...
namespace FTPClient
{
    ...
    public class FTPclient
    {
        ...
        public async Task<List<string>> ListDirectory(string directory)
        {
            //return a simple list of filenames in directory
            System.Net.FtpWebRequest ftp = await GetRequest(GetDirectory(directory));
            //Set request to do simple list
            ftp.Method = System.Net.WebRequestMethods.Ftp.ListDirectory;

            string str = await GetStringResponse(ftp);
        ...
        public async Task<FTPdirectory> ListDirectoryDetail(string directory)
        {
            string dir = GetDirectory(directory);
            FtpWebRequest ftp = await GetRequest(dir);
            //Set request to do simple list
        ...
        public async Task<bool> Download(string sourceFilename, string localFilename, bool PermitOverwrite)
        {
            //2. determine target file
            FileInfo fi = new FileInfo(localFilename);
            return await Download(sourceFilename, fi, PermitOverwrite);
        }

        //Version taking an FtpFileInfo
        public async Task<bool> Download(FTPfileInfo file, string localFilename, bool PermitOverwrite)
        {
            return await Download(file.FullName, localFilename, PermitOverwrite);
        }
        ...
        public async Task<bool> Download(string sourceFilename, FileInfo targetFI, bool PermitOverwrite)
        {
            //1. check target
            ...
            //3. perform copy
            System.Net.FtpWebRequest ftp = await GetRequest(URI);
        ...
        public bool FtpDelete(string filename)
        {
            ...
            try
            {
                ...
                string str = GetStringResponse(ftp).Result;
            }
            catch (Exception)
            {
                return false;
        ...
        public long GetFileSize(string filename)
        {
            ...
            string tmp = this.GetStringResponse(ftp).Result;
            return GetSize(ftp);
        }
        ...
        private async Task<FtpWebRequest> GetRequest(string URI)
        {
            //create request
        ...
        private async Task<string> GetStringResponse(FtpWebRequest ftp)
        {
    ...
    public class FTPfileInfo
    {
        ...
        private static string[] _ParseFormats = new string[] {
            @"(?<dir>[\-l])(?<permission>([\-r][\-w][\-xs]){3})\s+\d+\s+\w+\s+\w+\s+(?<size>\d+)\s+(?<timestamp>\w+\s+\d+\s+\d{2}[:]\d{2})\s+(?<name>[a-zA-Z0-9-_]+)",
    ...
    public class FTPdirectory : List<FTPfileInfo>
    {
        ...
        public static string GetParentDirectory(string dir)
        {
            ...
            {
                throw new Exception("No parent for root");
            }
        }
    }
    #endregion
